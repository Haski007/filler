...
static void         heat_coord(t_map *map, int y, int x, int value)
{
    if (y > 0)
    {
        if (!ft_strstr(map->en, &map->map[y - 1][x - 1]) && !map->heat[y - 1][x - 1])
            map->heat[y - 1][x - 1] = value;
        if (!ft_strstr(map->en, &map->map[y - 1][x]) && !map->heat[y - 1][x])
            map->heat[y - 1][x] = value;
        if (!ft_strstr(map->en, &map->map[y - 1][x + 1]) && !map->heat[y - 1][x + 1])
            map->heat[y - 1][x + 1] = value;
    }
    if (!ft_strstr(map->en, &map->map[y][x - 1]) && !map->heat[y][x - 1])
...
void                heat_map(t_map *map)
{
    ...
    while (++value < map->cols)
    {
        ...
        while (++y < map->rows)
        {
            ...
            while (++x < map->cols)
            {
                if ((map->map[y][x] == map->en[0] || map->map[y][x] == map->en[1])
                && map->heat[y][x] != -1)
                {
                    map->heat[y][x] = -1;
